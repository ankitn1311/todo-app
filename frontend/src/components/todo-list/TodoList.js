...
import { clearCurrentTodo, createTodo, getTodos, updateTodo } from "../../redux/actions/todos";
...
const TodoList = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if(currentTodo){
        const payload = {_id: currentTodo._id, title: values.title, description: values.description}
    ...
    <div className="flex items-start justify-center h-full p-4">
      <Paper className="w-full overflow-hidden rounded-xl sm:w-1/2 lg:w-1/3 xl:w-1/3">
        ...
        <div className="flex flex-col divide-y-1 divide-slate-700">
          {todos.map((todo) => (
            <Todo todo={todo} setButtonName={setButtonName}/>
          ))}
        </div>
      </Paper>
    </div>
